...
getgenv().jaidenConnection.heartBeat = game:GetService('RunService').Heartbeat:Connect(function()
    ...
    if ball then
        maxVelocity = ball.Velocity:Dot((character.HumanoidRootPart.Position - ball.Position).Unit)
        ...
        if character:FindFirstChild("Highlight") then
            if (character.HumanoidRootPart.Position - ball.Position).Magnitude <= (maxVelocity * 3/5 + 5) then
                if isParried(ball) < 0.46 then
                    parryButton:Fire()
                end
            end
        end
...
--[[
    loadstring(game:HttpGet("https://raw.githubusercontent.com/JoshaDeveloper/Roblox/main/cac.lua"))()
]]
